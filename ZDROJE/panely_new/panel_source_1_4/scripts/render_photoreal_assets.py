...
CONTROL_VALIDATION_PATH = ROOT / "output" / "validation" / "panel_1_4_validation.json"
...
def cutoff_for_group(group: int, max_group: int) -> float:
    return 900.0 if max_group == 0 else 610.0 + (290.0 * group / max_group)
    ...
    for obj in objects:
        ...
        if obj.get("geometry_kind") == "bore_marker":
            ...
            if state == "exploded":
                ...
                z = min(float(obj["z1_mm"]), cutoff_for_group(group, max_group)) + 0.35
            elif state == "full":
                z = float(obj["z1_mm"]) + 0.35
            else:
                raise ValueError(f"Unknown render state: {state}")
            ...
            cell = 2.0
            steps = int(math.ceil((2.0 * radius) / cell))
            for ix in range(steps):
                for iy in range(steps):
                    x = center_x - radius + ix * cell
                    y = center_y - radius + iy * cell
                    cx = x + cell / 2.0
                    cy = y + cell / 2.0
...
def create_review_sheet(panel: dict, material_library: dict, images: dict[str, Image.Image], validation: dict, output: Path) -> None:
    ...
    draw.text((56, 32), "PANEL 1.4  ·  FOTOREALISTICKÝ STANDARD", fill="#ffffff", font=font(44, True))
    draw.text((3144, 45), "PREFA ŠOPÍK · MATERIÁLY 03", anchor="ra", fill="#76e0d2", font=font(25, True))

    cards = [
        (50, 190, 1030, 1240, "Plná geometrie", images["full"]),
        (1110, 190, 2090, 1240, "Stupňovitý řez lamelami", images["cutaway"]),
        (2170, 190, 3150, 1240, "Rozložené lamely CLT", images["exploded"]),
    ]
    ...
    material_ids = ["clt_spruce_visual", "clt_spruce_transverse", "clt_spruce_nonvisual", "clt_bore_shadow"]
    labels = ["CLT · pohledová VI", "CLT · příčná lamela", "CLT · nepohledová NVI", "Vývrt + chránička"]
    ...
    layer_count = validation["logical_layer_count"]
    layer_word = "vrstva" if layer_count == 1 else "vrstev"
    draw.text((1320, 2070), f"{layer_count} {layer_word} · 2 stavy", fill="#172522", font=font(27, True))
...
def main() -> None:
    ...
    electrical_scope_complete = (
        ...
        and electrical.get("mounting_cavity_present") is False
        and electrical.get("host_layer_ids") == ["L01"]
        and "elektroinstalační chráničky vložené do vývrtů pro vedení elektroinstalace" in electrical.get("included_items", [])
        and "vývrty pro následnou montáž elektroinstalačních krabic" in electrical.get("included_items", [])
        and "elektroinstalační krabice" in electrical.get("excluded_items", [])
    )
    if not electrical_scope_complete:
        raise ValueError("Incomplete or inconsistent CLT electrical preparation scope")
    ...
    save_png_webp(full, OUT / "panel_1_4_photoreal_full")
    save_png_webp(cutaway, OUT / "panel_1_4_photoreal_assembled_cutaway")
    save_png_webp(exploded, OUT / "panel_1_4_photoreal_exploded")
    ...
    validation = {
        ...
        "continuous_visual_face_without_seams": True,
        "electrical_preview_bore_count": len(panel["electrical_preview_standard"]["bores"]),
        "electrical_preparation_documented": True,
        "electrical_conduits_included": panel["electrical_preview_standard"].get("conduit_included") is True,
        "box_mounting_bores_included": electrical_scope_complete,
    }
    (OUT / "panel_1_4_photoreal_validation.json").write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = {
        "panel_id": panel["panel_id"],
        "geometry_sha256": control_validation["geometry_sha256"],
        "composites": {
            "full_png": "output/photoreal/panel_1_4_photoreal_full.png",
            "full_webp": "output/photoreal/panel_1_4_photoreal_full.webp",
            "assembled_png": "output/photoreal/panel_1_4_photoreal_assembled_cutaway.png",
            "assembled_webp": "output/photoreal/panel_1_4_photoreal_assembled_cutaway.webp",
            "exploded_png": "output/photoreal/panel_1_4_photoreal_exploded.png",
            "exploded_webp": "output/photoreal/panel_1_4_photoreal_exploded.webp",
            "layer_table_png": "output/panel_1_4_vrstvy_a_vlastnosti.png"
        },
        "visual_rules": {
            "interior_face": "Souvislá celistvá plocha masivního dřeva bez viditelných spár, drážek nebo rastru prken.",
            "cross_lamination_visibility": "Pouze na řezu, hranách a v rozloženém stavu."
        },
        "electrical_preview": panel["electrical_preview_standard"],
    ...
    (OUT / "panel_1_4_asset_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    create_review_sheet(panel, material_library, {"full": full, "cutaway": cutaway, "exploded": exploded}, validation, OUT / "panel_1_4_photoreal_review.png")
