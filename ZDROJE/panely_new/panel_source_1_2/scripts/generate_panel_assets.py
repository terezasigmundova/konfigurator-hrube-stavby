...
def render_scene(panel: dict, standard: dict, objects: list[dict], output_path: Path, exploded: bool) -> None:
    ...
    for obj in objects:
        ...
        poly = Poly3DCollection(
            ...
            linewidths=0.75 if obj["layer_id"] != "L05" else 1.4,
        )
...
def draw_cross_section(draw: ImageDraw.ImageDraw, panel: dict, layer_ranges: dict[str, tuple[float, float]], area: tuple[int, int, int, int]) -> None:
    ...
    draw.text((x0, y0 + 45), "Hostované izolace L03 a L07 leží uvnitř rámů a nepřičítají se podruhé.", fill="#5f706d", font=font(22))
...
def create_proof_sheet(panel: dict, standard: dict, layer_ranges: dict[str, tuple[float, float]], assembled: Path, exploded: Path, output: Path, validation: dict) -> None:
    ...
    draw.text((56, 33), "PANEL 1.2  ·  KONTROLA GEOMETRIE", fill="#ffffff", font=font(46, True))
    ...
    rows = [
        ...
        ("Instalační dutina", "40 mm"),
        ("Nosné KVH", "60 × 240 mm"),
        ("Nosný rám", "240 mm"),
        ("Difuzní membrána", "0,5 mm"),
        ("Dvojitý podkladní rošt", "2 × 40 mm"),
        ("Celková tloušťka", "398,5 mm"),
    ]
    y = 284
...
def main() -> None:
    ...
    checks = {
        ...
        "service_frame_60x40": any(
            layer["id"] == "L02" and math.isclose(float(layer["member_face_width_mm"]), 60.0) and math.isclose(float(layer["member_depth_mm"]), 40.0)
            for layer in panel["layers"]
        ),
        "structural_frame_60x240": any(
            layer["id"] == "L06" and math.isclose(float(layer["member_face_width_mm"]), 60.0) and math.isclose(float(layer["member_depth_mm"]), 240.0)
            for layer in panel["layers"]
        ),
        "double_substructure_60x40": all(
            any(
                layer["id"] == layer_id
                and layer["geometry"] == "frame"
                and math.isclose(float(layer["member_face_width_mm"]), 60.0)
                and math.isclose(float(layer["member_depth_mm"]), 40.0)
                and layer.get("member_axis") == axis
                for layer in panel["layers"]
            )
            for layer_id, axis in (("L10", "vertical"), ("L11", "horizontal"))
        ),
        "no_facade_boards": all("prk" not in layer["name"].lower() and "obklad" not in layer["name"].lower() for layer in panel["layers"]),
    ...
    assembled_path = OUT_RENDERS / "panel_1_2_geometry_assembled.png"
    exploded_path = OUT_RENDERS / "panel_1_2_geometry_exploded.png"
    ...
    write_obj(panel, objects, OUT_MODELS / "panel_1_2.obj")
    (OUT_VALIDATION / "panel_1_2_validation.json").write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    create_proof_sheet(
        panel,
        standard,
        layer_ranges,
        assembled_path,
        exploded_path,
        OUT_VALIDATION / "panel_1_2_geometry_control.png",
        validation,
    )
