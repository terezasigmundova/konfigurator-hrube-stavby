    ...
    for layer in sorted(panel["layers"], key=lambda item: item["order"]):
        ...
        if geometry == "solid":
            ...
            if layout == "single_centered_member":
                ...
                objects.append({**base, "name": f"{layer_id}_stud_center", "bounds": [member_x0, member_x1, y0, y1, 0.0, height]})
...
def create_proof_sheet(panel: dict, standard: dict, layer_ranges: dict[str, tuple[float, float]], assembled: Path, exploded: Path, output: Path, validation: dict) -> None:
    ...
    draw.text((56, 33), "PANEL 1.1  ·  KONTROLA GEOMETRIE", fill="#ffffff", font=font(46, True))
    ...
    rows = [
        ...
        ("Nosné KVH", "60 × 160 mm"),
        ("Nosný rám", "160 mm"),
        ("Fasádní izolace", "100 mm"),
        ("Armovací vrstva", "3 mm"),
        ("Celková tloušťka", "341,0 mm"),
    ]
    y = 284
...
def main() -> None:
    ...
    checks = {
        ...
        "structural_frame_60x160": any(
            layer["id"] == "L06" and math.isclose(float(layer["member_face_width_mm"]), 60.0) and math.isclose(float(layer["member_depth_mm"]), 160.0)
            for layer in panel["layers"]
        ),
    ...
    assembled_path = OUT_RENDERS / "panel_1_1_geometry_assembled.png"
    exploded_path = OUT_RENDERS / "panel_1_1_geometry_exploded.png"
    ...
    write_obj(panel, objects, OUT_MODELS / "panel_1_1.obj")
    (OUT_VALIDATION / "panel_1_1_validation.json").write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    create_proof_sheet(
        panel,
        standard,
        layer_ranges,
        assembled_path,
        exploded_path,
        OUT_VALIDATION / "panel_1_1_geometry_control.png",
        validation,
    )
