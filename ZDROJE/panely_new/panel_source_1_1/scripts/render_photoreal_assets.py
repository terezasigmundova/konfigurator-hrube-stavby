...
PANEL_PATH = ROOT / "data" / "panel_1_1.json"
STANDARD_PATH = ROOT / "data" / "render_standard.json"
MATERIAL_PATH = ROOT / "data" / "material_library.json"
CONTROL_VALIDATION_PATH = ROOT / "output" / "validation" / "panel_1_1_validation.json"
...
def object_bounds_for_state(obj: dict, state: str, gap: float) -> list[float]:
    ...
    if state == "exploded":
        ...
        cutoffs = {0: 610.0, 1: 650.0, 2: 690.0, 3: 720.0, 4: 765.0, 5: 810.0, 6: 855.0, 7: 900.0}
        bounds[5] = min(bounds[5], cutoffs[int(obj["exploded_group"])])
    elif state != "full":
...
def build_faces(objects: list[dict], state: str, projector: Projector, gap: float, only_layer: str | None = None) -> list[dict]:
    ...
    for obj in objects:
        if only_layer and obj["layer_id"] != only_layer:
            continue
        bounds = object_bounds_for_state(obj, state, gap)
...
def create_review_sheet(panel: dict, material_library: dict, images: dict[str, Image.Image], validation: dict, output: Path) -> None:
    ...
    draw.text((56, 32), "PANEL 1.1  ·  FOTOREALISTICKÝ STANDARD", fill="#ffffff", font=font(44, True))
    draw.text((3144, 45), "PREFA ŠOPÍK · MATERIÁLY 01", anchor="ra", fill="#76e0d2", font=font(25, True))
    ...
    material_ids = ["fermacell_raw", "kvh_spruce", "mineral_wool_soft", "vapour_membrane", "mineral_wool_facade", "reinforced_basecoat"]
    labels = ["Fermacell", "KVH smrk", "Minerální izolace", "Parozábrana", "Fasádní izolace", "Armovací stěrka"]
...
def main() -> None:
    ...
    if not math.isclose(total, 341.0, abs_tol=1e-6):
    ...
    save_png_webp(full, OUT / "panel_1_1_photoreal_full")
    save_png_webp(cutaway, OUT / "panel_1_1_photoreal_assembled_cutaway")
    save_png_webp(exploded, OUT / "panel_1_1_photoreal_exploded")
    ...
    (OUT / "panel_1_1_photoreal_validation.json").write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = {
        "panel_id": panel["panel_id"],
        "geometry_sha256": control_validation["geometry_sha256"],
        "composites": {
            "full_png": "output/photoreal/panel_1_1_photoreal_full.png",
            "assembled_png": "output/photoreal/panel_1_1_photoreal_assembled_cutaway.png",
            "assembled_webp": "output/photoreal/panel_1_1_photoreal_assembled_cutaway.webp",
            "exploded_png": "output/photoreal/panel_1_1_photoreal_exploded.png",
            "exploded_webp": "output/photoreal/panel_1_1_photoreal_exploded.webp"
        },
    ...
    (OUT / "panel_1_1_asset_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    create_review_sheet(panel, material_library, {"full": full, "cutaway": cutaway, "exploded": exploded}, validation, OUT / "panel_1_1_photoreal_review.png")
