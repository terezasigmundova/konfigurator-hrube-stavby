...
CONTROL_VALIDATION_PATH = ROOT / "output" / "validation" / "panel_1_3_validation.json"
...
def create_review_sheet(panel: dict, material_library: dict, images: dict[str, Image.Image], validation: dict, output: Path) -> None:
    ...
    draw.text((56, 32), "PANEL 1.3  ·  FOTOREALISTICKÝ STANDARD", fill="#ffffff", font=font(44, True))
    ...
    material_ids = ["fermacell_raw", "kvh_spruce", "mineral_wool_soft", "vapour_membrane"]
    labels = ["Fermacell", "KVH smrk", "Minerální izolace", "Parozábrana"]
    ...
    completion = panel["customer_completion"]
    draw.rounded_rectangle((50, 1705, 3150, 1955), radius=20, fill="#fff7e6", outline="#e8c87b", width=3)
    draw.text((82, 1735), completion["user_heading"].upper(), fill="#8a5b08", font=font(24, True))
    wrapped(draw, (82, 1782), completion["user_description"], 3000, "#3b3325", font(22), spacing=6)
    client_items = "Klient dodá: " + " · ".join(completion["client_supplied_items"]) + "."
    wrapped(draw, (82, 1885), client_items, 3000, "#6f5b37", font(20, True), spacing=4)
...
def main() -> None:
    ...
    completion = panel["customer_completion"]
    scope_complete = (
        panel.get("electrical_preparation", {}).get("included") is False
        and panel.get("electrical_preparation", {}).get("responsibility") == "client"
        and completion.get("required") is True
        and completion.get("host_layer_ids") == ["L01"]
        and len(completion.get("client_supplied_items", [])) == 4
    )
    if not scope_complete:
        raise ValueError("Incomplete or inconsistent customer completion scope")
    ...
    save_png_webp(full, OUT / "panel_1_3_photoreal_full")
    save_png_webp(cutaway, OUT / "panel_1_3_photoreal_assembled_cutaway")
    save_png_webp(exploded, OUT / "panel_1_3_photoreal_exploded")

    managed_dirs = [
        OUT / "layers" / "assembled",
        OUT / "layers" / "exploded",
        OUT / "masks" / "assembled",
        OUT / "masks" / "exploded",
    ]
    for managed_dir in managed_dirs:
        ...
        for stale_file in managed_dir.iterdir():
            if stale_file.is_file() and stale_file.suffix.lower() in {".webp", ".png"}:
                stale_file.unlink()
    ...
    validation = {
        ...
        "electrical_preparation_included": False,
        "customer_completion_documented": scope_complete,
    }
    (OUT / "panel_1_3_photoreal_validation.json").write_text(json.dumps(validation, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    manifest = {
        "panel_id": panel["panel_id"],
        "geometry_sha256": control_validation["geometry_sha256"],
        "composites": {
            "full_png": "output/photoreal/panel_1_3_photoreal_full.png",
            "assembled_png": "output/photoreal/panel_1_3_photoreal_assembled_cutaway.png",
            "assembled_webp": "output/photoreal/panel_1_3_photoreal_assembled_cutaway.webp",
            "exploded_png": "output/photoreal/panel_1_3_photoreal_exploded.png",
            "exploded_webp": "output/photoreal/panel_1_3_photoreal_exploded.webp",
            "layer_table_png": "output/panel_1_3_vrstvy_a_vlastnosti.png"
        },
        "layers": layers_manifest,
        "layer_definitions": [
            {
                "id": layer["id"],
                "derived_from_layer_id": layer.get("derived_from_layer_id"),
                "name": layer["name"],
        ...
        "features": [
            {
                ...
                **completion,
                "electrical_preparation": panel["electrical_preparation"],
                "interaction": {
                    "hover_target_layer_ids": completion["host_layer_ids"],
                    "on_select": "show_customer_completion_description"
                }
            }
        ]
    }
    (OUT / "panel_1_3_asset_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    create_review_sheet(panel, material_library, {"full": full, "cutaway": cutaway, "exploded": exploded}, validation, OUT / "panel_1_3_photoreal_review.png")
