...
OUT = ROOT / "output" / "panel_1_3_vrstvy_a_vlastnosti.png"
...
def main() -> None:
    ...
    customer_y = footer_y + 195
    width, height = 3200, customer_y + 340
    image = Image.new("RGB", (width, height), "#f3f6f5")
    draw = ImageDraw.Draw(image)

    draw.rectangle((0, 0, width, 150), fill="#0f172a")
    draw.text((56, 32), "PANEL 1.3  ·  VRSTVY A VLASTNOSTI", fill="#ffffff", font=font(44, True))
    ...
    total = f"{panel['declared_delivered_thickness_mm']:.1f} mm".replace(".", ",")
    ...
    draw.text((600, footer_y + 65), "Izolace L05 je uvnitř nosného rámu L04 a nepřičítá se podruhé.", fill="#172522", font=font(24, True))

    completion = panel["customer_completion"]
    draw.rounded_rectangle((55, customer_y, 3145, customer_y + 280), radius=20, fill="#fff7e6", outline="#e8c87b", width=3)
    draw.text((85, customer_y + 25), completion["user_heading"].upper(), fill="#8a5b08", font=font(23, True))
    wrapped(draw, (85, customer_y + 70), completion["user_description"], 3000, "#3b3325", font(21), spacing=6)
    client_items = "Klient dodá: " + " · ".join(completion["client_supplied_items"]) + "."
    wrapped(draw, (85, customer_y + 170), client_items, 3000, "#6f5b37", font(19, True), spacing=4)
    draw.text((55, height - 40), "Zdroj: data/panel_1_3.json · rozměry v mm · společný zobrazovací standard PREFA_SOPIK_RENDER_STD_01", fill="#64736f", font=font(18))
